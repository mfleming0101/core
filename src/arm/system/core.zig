...
/// What one part was built with that its core's TRM leaves to it: the level 1 caches of the M7, M55 and M85, the M7 TCMs, AHBP and ECC of M7 TRM Table 1-1, the MPU and SAU region counts, the priority bits, the external interrupts, the REVIDRNUM the M55 and M85 read in REVIDR, M55 and M85 TRM 5.7, and the vector tables VTOR resets to, from INITVTOR on the M7, M7 TRM Table 3-1, from the pins of the M23, M23 TRM Table 5-1, and from INITSVTOR and INITNSVTOR on the M33, M55 and M85, M33 TRM Table 3-1, M55 and M85 TRM Table 5-1, and SYST_CALIB, of which the SKEW and TENMS bits are kept as CFGSTCALIB wires them, M7 TRM Table 3-2, and CFGSSTCALIB of M55 and M85 TRM C.3, where NOREF always reads one as the library fits no reference clock, and SYST_CALIB_NS the same from CFGNSSTCALIB, M55 and M85 TRM C.3, of the Non-secure SysTick that the M33, M55 and M85 always fit, v8-M D1.2.240, and the M23 fits where `systick_ns` is set, M23 TRM Table 2-1; a core that carries none ignores them, and a value left null is the core's default.
