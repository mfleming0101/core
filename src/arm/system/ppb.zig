...
pub const Region = enum { memory, systick, systick_ns, control, control_ns, scb, scb_ns, revidr, revidr_ns, nvic, nvic_ns, itm, dwt, ras, impdef, impdef_ns, ewic, ppb_unmapped };
...
pub fn region(address: u32) Region {
    ...
    if (address -% (impdef.base + alias) < impdef.size) return .impdef_ns;
